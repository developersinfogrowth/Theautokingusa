...
const INITIAL: FormState = {
  slug: "", type: "engine", title: "",
  description: "", warranty: "", badge: "",
  image: "", image_preview: "",
  seoTitle: "", seoDescription: "", keywords: "",
  rich_content: "", status: "published",
};
...
export default function BrandPageForm({ id }: BrandPageFormProps) {
  ...
  const urlPreview = form.slug && form.type
    ? `/${
        form.type === "engine"       ? "used-engines"       :
        form.type === "transmission" ? "used-transmissions" :
        form.type
      }/${form.slug}`
    : null;
  ...
  const handleImageFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (errs.length) { setImageErrors(errs); return; }
    setImageErrors([]);

    const reader = new FileReader();
    reader.onload = ev => set("image_preview", ev.target?.result as string);
    ...
    try {
      ...
      setImageErrors([err instanceof Error ? err.message : "Upload failed"]);
      set("image_preview", "");
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let result;
    if (isEditMode && id) {
      result = await supabase.from("autoking_brands").update(payload).eq("id", id);
    } else {
      result = await supabase.from("autoking_brands").insert(payload);
    }
    ...
    if (isEditMode) {
      setTimeout(() => router.push("/brands"), 900);
    } else {
      setForm(INITIAL);
      if (fileInputRef.current) fileInputRef.current.value = "";
  ...
  if (loadError) {
    return (
      <div className="max-w-3xl mx-auto py-20 px-4 text-center">
        ...
        <Link href="/brands" className="text-sm font-medium text-gray-700 hover:text-gray-900">
    ...
    <div className="max-w-3xl mx-auto py-8 px-4">
      ...
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-gray-900">
          {isEditMode ? "Edit Brand Page" : "Add Engine / Transmission Page"}
        </h1>
      ...
      {submitStatus === "success" && (
        <div className="mb-5 flex items-center gap-3 bg-green-50 border border-green-200
                        text-green-700 px-4 py-3 rounded-lg text-sm">
          <CheckCircle className="w-4 h-4 shrink-0" />
          {isEditMode ? "Changes saved!" : "Page saved successfully!"}
          {urlPreview && (
            <code className="ml-1 font-mono text-green-600">{urlPreview}</code>
          )}
          {!isEditMode && (
            <Link href="/brands" className="ml-auto text-xs font-medium underline">
              View all brands
            </Link>
          )}
        </div>
      ...
      {submitStatus === "error" && (
        <div className="mb-5 flex items-center gap-3 bg-red-50 border border-red-200
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className={sectionClass}>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              />
              {urlPreview && (
                <p className="text-xs text-green-600 mt-1 font-mono bg-green-50 px-2 py-1 rounded">
                  → {urlPreview}
                </p>
              )}
            </div>
            ...
            <div>
              <label className={labelClass}>Card Title <span className="text-red-500">*</span></label>
              <input
                name="title" required
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange} value={form.warranty}
                className={inputClass}
              />
            </div>
              ...
              {form.image_preview ? (
                <div className="relative border-2 border-green-400 rounded-xl overflow-hidden bg-gray-50">
                  ...
                  {form.image && !uploadingImage && (
                    <div className="absolute bottom-2 left-2 bg-green-600 text-white
                ...
                <label className="flex flex-col items-center justify-center gap-2 p-8
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className={`text-xs ${form.seoTitle.length > 60 ? "text-red-500 font-medium" : "text-gray-400"}`}>
              ...
              <input
                ...
                onChange={handleChange} value={form.seoTitle}
                className={inputClass}
              />
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className={`text-xs ${form.seoDescription.length > 160 ? "text-red-500 font-medium" : "text-gray-400"}`}>
              ...
              <textarea
                ...
                onChange={handleChange} value={form.seoDescription}
                className={inputClass}
              />
              <p className="text-xs text-gray-400 mt-1">
                Shown as the grey text under the title in Google search results
              </p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange} value={form.keywords}
                className={inputClass}
              />
              <p className="text-xs text-gray-400 mt-1">
        ...
        <div className={sectionClass}>
          ...
          <p className="text-xs text-gray-400 mb-3">
            Add your page content with headings, lists, links, and images.
            Use the toolbar to format your content or click the{" "}
            <code className="bg-gray-100 px-1 rounded">&lt;/&gt;</code> icon to edit raw HTML.
          </p>
          <RichTextEditor
        ...
        <button
          type="submit"
          disabled={submitStatus === "loading" || uploadingImage}
